...
class Classifiers:
    ...
    def evaluator(self, instances_directories):
        ...
        output = pd.DataFrame(results, columns=['time_window', 'abnormality_value', 'monitoring_method',
                                                ...
                                                'train_time'])
        output.to_csv(os.path.join(os.getcwd(), 'Early_Stopping', 'param-free-' + self.pattern_name + '.csv'))
...
class MultiObjectiveEarlyStopping(tf.keras.callbacks.Callback):
    ...
    """
    def __init__(self, patience, minimum_no_epochs=1, maximum_no_epochs=1000, verbosity=0):
...
class Experimenter:
    ...
    def experiment_conductor(self, imbalanced_ratio, pattern_name, subclass_weights, minimum_no_epochs,
                             maximum_no_epochs, patience, instances, method_name="BayesianOptimization"):
    ...
    def main(self):
        ...
        for pattern_name in pattern_names:
            ...
            file_name = os.path.join(os.getcwd(), 'Early_Stopping', f'param-free-{pattern_name}.csv')
        ...
        concatenated_df.to_csv(os.path.join(os.getcwd(), "Early_Stopping", 'param-free-early_stoppings.csv'),
                               index=False)
